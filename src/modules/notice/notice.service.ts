...
import { NoticeItemDto, NoticeResponseDto, NoticeDetailResponseDto } from './dto/notice.dto';
...
export class NoticeService {
   ...
   * @param keyword 검색어
   */
  async getNoticeList(page = 1, searchType = 'all', keyword = ''): Promise<NoticeResponseDto> {
    try {
      // 요청 URL 구성
      const url = this.buildListUrl(page, searchType, keyword);
  ...
  async getNoticeDetail(nttId: string): Promise<NoticeDetailResponseDto> {
    try {
      ...
      // 디버깅 로그 추가
      this.logger.debug(`파싱 결과: 제목="${detail.title}", 내용 길이=${detail.content?.length || 0}`);
      ...
      this.logger.error(`공지사항 상세 정보 조회 실패: ${error.message}`, error.stack);
      throw new Error(`공지사항 상세 정보 조회 중 오류가 발생했습니다: ${error.message}`);
    }
  }
   ...
   * @param keyword 검색어
   */
  async getRegularNoticeList(page = 1, searchType = 'all', keyword = ''): Promise<NoticeResponseDto> {
    try {
      // 요청 URL 구성
      const url = this.buildListUrl(page, searchType, keyword);
   ...
   * @param keyword 검색어
   */
  async getNewNoticeList(page = 1, searchType = 'all', keyword = ''): Promise<NoticeResponseDto> {
    try {
      // 요청 URL 구성
      const url = this.buildListUrl(page, searchType, keyword);
   ...
   * @param keyword 검색어
   */
  async getFeaturedNoticeList(page = 1, searchType = 'all', keyword = ''): Promise<NoticeResponseDto> {
    try {
      // 요청 URL 구성
      const url = this.buildListUrl(page, searchType, keyword);
   ...
   * @returns 공지사항 목록
   */
  async getTodayNoticeList(page: number, searchType: string, keyword: string): Promise<NoticeResponseDto> {
    try {
      const url = this.buildListUrl(page, searchType, keyword);
      const html = await this.fetchHtml(url);
      ...
      response.items = response.items.filter(item => 
        item.date === todayString && item.no !== '공지'
      );
      
      this.logger.log(`해당 날짜 등록된 공지사항 (no가 숫자인 항목만): ${response.items.length}개`);
      return response;
    } catch (error) {
      this.logger.error(`오늘 공지사항 목록 조회 중 오류 발생: ${error.message}`);
   ...
   * 목록 URL을 구성합니다.
   * @param page 페이지 번호
   * @param searchType 검색 유형 (무시됨)
   * @param keyword 검색어 (무시됨)
   * @returns 구성된 URL
   */
  private buildListUrl(page: number, searchType: string, keyword: string): string {
  ...
  private async fetchHtml(url: string): Promise<string> {
    try {
      ...
      this.logger.debug(`응답 상태 코드: ${response.status}, 컨텐츠 타입: ${response.headers['content-type']}`);
   ...
   * @returns 파싱된 공지사항 목록
   */
  private parseNoticeList(html: string, currentPage: number): NoticeResponseDto {
  ...
  private extractNoticeItems($: cheerio.CheerioAPI): NoticeItemDto[] {
    ...
    $('.board_list tbody tr').each((index, element) => {
      ...
      const link = nttId ? 
        `${HANBAT_NOTICE.VIEW_URL}?nttId=${nttId}` : '';
   ...
   */
  private parseNoticeDetail(html: string, nttId: string): NoticeDetailResponseDto {
    try {
      ...
      if ($actualContent.length > 0) {
        ...
        $plainContentEl.find('p, div, h1, h2, h3, h4, h5, h6, li').each((_, el) => {
          $(el).append('\n');
        });
        
        // 순수 텍스트 추출
        content = $plainContentEl.text()
          .replace(/\n\s*\n/g, '\n\n') // 중복 줄바꿈 제거
          .replace(/[ \t]+/g, ' ')     // 중복 공백 제거
          .trim();
        
        this.logger.debug(`순수 텍스트 내용 추출 완료, 길이: ${content.length}`);
      ...
      if (!content) {
        ...
        $plainTextEl.find('p, div, h1, h2, h3, h4, h5, h6, li').each((_, el) => {
          $(el).append('\n');
        });
        
        content = $plainTextEl.text().replace(/\n\s*\n/g, '\n\n').trim();
      }
      ...
      $('.ui.bbs--view--file a.btn-file').each((i, el) => {
        ...
        if (href.includes('javascript:fn_egov_downFile')) {
          // 정규식으로 파일 ID와 순번 추출
          const fileIdMatch = href.match(/fn_egov_downFile\('([^']+)',\s*'(\d+)'\)/);
          ...
          link = href.startsWith('/') ? `https://www.hanbat.ac.kr${href}` : href;
        }
      ...
      // 작성자 추출
      let author = '';
      const authorText = $('.ui.bbs--view--header span:contains("작성자")').text().trim();
      if (authorText) {
