...
export class MenuService {
  ...
  private readonly baseUrl = 'https://www.hanbat.ac.kr/prog/carteGuidance/kor/sub06_030301/C1/calendar.do';
  private readonly ajaxUrl = 'https://www.hanbat.ac.kr/prog/carteGuidance/kor/sub06_030301/C1/getCalendar.do';
  ...
  async getMenuByDate(date?: string): Promise<MenuResponseDto> {
    try {
      ...
      // 해당 날짜의 메뉴 아이템 추출
      const menuItems = this.extractMenuItemsFromData(menuData, formattedDate, dayIndex);
  ...
  async getWeeklyMenu(startDate?: string): Promise<MenuResponseDto[]> {
    try {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        // 해당 요일의 메뉴 아이템 추출
        const menuItems = this.extractMenuItemsFromData(menuData, formattedDate, i);
  ...
  private async fetchMenuDataFromAjax(mondayDate: string): Promise<any> {
    try {
      ...
      if (response.data && response.data.item && Array.isArray(response.data.item)) {
        this.logger.log(`메뉴 데이터 가져오기 성공: ${response.data.item.length}개 항목`);
   ...
   */
  private extractMenuItemsFromData(menuData: any[], date: string, dayIndex: number): MenuItemDto[] {
   ...
   * @param date 날짜 문자열
   */
  private formatMenuResponse(menuItems: MenuItemDto[], date: string): MenuResponseDto {
    const lunchMenu = menuItems.find(item => item.meal === 'lunch')?.menu || [];
    const dinnerMenu = menuItems.find(item => item.meal === 'dinner')?.menu || [];
